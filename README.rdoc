...
Please refer to the {Benchmark Email API Documentation}[http://www.benchmarkemail.com/API/Library] for information about what methods are available, the arguments that should be passed to each method, and the output that the API will return. Start by creating a Benchwarmer object:

  b = Benchwarmer::API.new('username', 'password')

Don't forget that your "username" can also be your email address, depending on how you have set up your Benchmark Email account. You can also override the default configuration by passing in your own when initializing:

  b = Benchwarmer::API.new('username', 'password, {:secure => true, :timeout => 60})
